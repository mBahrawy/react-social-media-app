...
export const getPosts = createAsyncThunk<Post[], "", { rejectValue: FetchError }>("Post/all", async (_, thunkAPI) => {
  try {
...
export const getPost = createAsyncThunk<Post, string, { rejectValue: FetchError }>("Post/single", async (id, thunkAPI) => {
  try {
    const { getRequest } = new HttpService();
...
export const getPostComments = createAsyncThunk<Comment[], string, { rejectValue: FetchError }>("Post/comments", async (id, thunkAPI) => {
  try {
    const { getRequest } = new HttpService();
...
export const createPost = createAsyncThunk<Post, unknown, { rejectValue: FetchError }>("Post/create", async (data, thunkAPI) => {
  try {
...
export const postSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getPostComments.fulfilled, (state: PostsSliceState, action: PayloadAction<Comment[]>) => {
